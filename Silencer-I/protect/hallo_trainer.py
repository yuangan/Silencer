...
class VideoModelTrainer(nn.Module):
    def __init__(self, cfg):
        ...
        """
        self.cfg = cfg
        self.kwargs = DistributedDataParallelKwargs(find_unused_parameters=False)
        self.accelerator = Accelerator(
            gradient_accumulation_steps=cfg.solver.gradient_accumulation_steps,
            mixed_precision=cfg.solver.mixed_precision,
            log_with="mlflow",
            project_dir="./mlruns",
            kwargs_handlers=[self.kwargs],
        )
        ...
        self.num_update_steps_per_epoch = math.ceil(
            len(self.train_dataloader) / cfg.solver.gradient_accumulation_steps
        )
        self.num_train_epochs = math.ceil(
            cfg.solver.max_train_steps / self.num_update_steps_per_epoch
        )
    ...
    def initialize_models(self):
        ...
        self.denoising_unet = UNet3DConditionModel.from_pretrained_2d(
            self.cfg.base_model_path, self.cfg.mm_path, subfolder="unet", 
            unet_additional_kwargs=OmegaConf.to_container(self.cfg.unet_additional_kwargs),
            use_landmark=False).to(device="cuda", dtype=weight_dtype)
        ...
        self.face_locator = FaceLocator(
            conditioning_embedding_channels=320).to(device="cuda", dtype=weight_dtype)
        self.audioproj = AudioProjModel(
            seq_len=5, blocks=12, channels=768, intermediate_dim=512, 
            output_dim=768, context_tokens=32).to(device="cuda", dtype=weight_dtype)

        # Load pretrained weights from stage 1
        self.load_stage1_weights()

        # Freeze certain models
        self.freeze_models()

    def load_stage1_weights(self):
        """
        Loads the pretrained weights from stage 1 checkpoints.
        """
        stage1_ckpt_dir = self.cfg.stage1_ckpt_dir
        self.denoising_unet.load_state_dict(
            torch.load(os.path.join(stage1_ckpt_dir, "denoising_unet.pth"), map_location="cpu"), strict=False)
        self.reference_unet.load_state_dict(
            torch.load(os.path.join(stage1_ckpt_dir, "reference_unet.pth"), map_location="cpu"), strict=False)
        self.face_locator.load_state_dict(
            torch.load(os.path.join(stage1_ckpt_dir, "face_locator.pth"), map_location="cpu"), strict=False)
        self.imageproj.load_state_dict(
            torch.load(os.path.join(stage1_ckpt_dir, "imageproj.pth"), map_location="cpu"), strict=False)

    def freeze_models(self):
        """
        Freezes the gradients for models that should not be updated during training.
        """
        ...
        self.audioproj.requires_grad_(True)

        # Unfreeze specific modules in denoising_unet
        trainable_modules = self.cfg.trainable_para
    ...
    def get_optimizer(self):
        ...
        """
        trainable_params = list(filter(lambda p: p.requires_grad, self.denoising_unet.parameters()))
    ...
    def forward(self, batch):
        ...
        if self.cfg.snr_gamma == 0:
            ...
            # Custom loss with SNR (Signal-to-Noise Ratio)
            snr = compute_snr(self.train_noise_scheduler, timesteps)
            snr = snr + 1
            mse_loss_weights = (torch.stack([snr, self.cfg.snr_gamma * torch.ones_like(timesteps)], dim=1).min(dim=1)[0]) / snr
            loss = (F.mse_loss(model_pred.float(), noise.float(), reduction="none") * mse_loss_weights).mean()
    ...
    def train(self):
        ...
        if self.cfg.resume_from_checkpoint:
            global_step = load_checkpoint(self.cfg, checkpoint_dir, self.accelerator)
            first_epoch = global_step // self.num_update_steps_per_epoch

        progress_bar = tqdm(range(global_step, self.cfg.solver.max_train_steps),
        ...
        for epoch in range(first_epoch, self.num_train_epochs):
            for step, batch in enumerate(self.train_dataloader):
                ...
                if self.accelerator.sync_gradients:
                    self.accelerator.clip_grad_norm_(self.optimizer.parameters(), self.cfg.solver.max_grad_norm)
                self.optimizer.step()
                ...
                if global_step % self.cfg.val.validation_steps == 0:
                    self.validate(global_step)

                if global_step >= self.cfg.solver.max_train_steps:
                    break
    ...
    def validate(self, global_step):
        """
        Runs the validation process during training.
        ...
        if self.accelerator.is_main_process:
            log_validation(
                ...
                vae=self.vae,
                net=self.net,
                scheduler=self.val_noise_scheduler,
                width=self.cfg.data.train_width,
                height=self.cfg.data.train_height,
                clip_length=self.cfg.data.n_sample_frames,
                cfg=self.cfg,
                save_dir=self.cfg.validation_dir,
                global_step=global_step,
            )

# Example usage:
# trainer = VideoModelTrainer(cfg)
# trainer.train()
