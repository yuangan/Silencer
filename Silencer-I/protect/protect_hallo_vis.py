...
class HalloNet(nn.Module):
    ...
    def visualize_latent(self, latents, outname='./latent_0_m_118752.png'):
        import matplotlib.pyplot as plt
        import cv2
        x = latents.squeeze(2).detach().cpu().numpy()
        for i in range(4):
            img = x[0, i, :, :]     # x：[b c h w]
            pmin = np.min(img)
            pmax = np.max(img)
            ...
            print(img.shape)        # (14, 14)
            img = cv2.applyColorMap(img, cv2.COLORMAP_JET)  # 生成heat map
            print(img.shape)        # (14, 14, 3)
            img = img[:, :, ::-1]  # 注意cv2（BGR）和matplotlib(RGB)通道是相反的
            plt.imsave(outname[:-4]+'_'+str(i)+'.png', img)
    ...
    def forward(self, batch):
        ...
        if self.iters == 92:
            self.visualize_latent(noisy_latents, f'./latent_{timesteps}_m_118752.png')
            assert(0)
...
def init(epsilon: int = 16, steps: int = 100, alpha: int = 1, 
    ...
    if mode == 'refnet':
        ...
        model = HalloRefNet('./configs/train/stage2.yaml')
        net = target_model_refnet(model, input_prompt, mode=mode, rate=rate, g_mode=g_mode)

    elif mode == 'hallo':
        print(' Define hallo model...')
        model = HalloNet('./configs/train/stage2.yaml', min_timesteps=min_ts)
...
def main(cfg : DictConfig):
    ...
    output_path = output_path + f'/{mode_name}_eps{epsilon}_steps{steps}_gmode{g_mode}_{min_ts}_11_15_vis'
    if diff_pgd[0]:
    ...
    if mode == 'refnet':
        ...
        for source_image_path in tqdm(source_image_paths):
            cprint(f'Processing: [{source_image_path}]', 'y')
            if not 'm_118752' in source_image_path:
                continue
