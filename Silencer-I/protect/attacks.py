...
class Linf_PGD_Hallo():
    ...
    def pgd_sds_refnet(self, X, net, c, label=None, random_start=False, delta_score=False, name='test', diff_pgd=False, using_target=False, target_info=None, target_rate=None):
        
        # add uniform random start [-eps, eps]
        X_adv = {}
        X_adv['image'] = X['image'].clone().detach() + (torch.rand(*X['image'].shape)*2*self.eps-self.eps).cuda()
        X_adv['face_emb'] = X['face_emb']
        ...
        for i in pbar:
            ...
            X_adv['image'] = X_adv['image'].detach()
            X_adv['image'].requires_grad_(True)

            loss = self.net(X_adv)

            pbar.set_description(f"[Running attack]: Loss {loss.item():.5f} | step size: {actual_step_size:.4}")

            grad, = torch.autograd.grad(loss, [X_adv['image']])
            
            # update
            X_adv['image'] = X_adv['image'] - grad.detach().sign() * actual_step_size
            # clip
            X_adv['image'] = torch.minimum(torch.maximum(X_adv['image'], X['image'] - self.eps), X['image'] + self.eps)
            X_adv['image'].data = torch.clamp(X_adv['image'], min=self.clip_min, max=self.clip_max)
            X_adv['image'].grad = None    
    ...
    def debug_memory(self):
        ...
        tensors = collections.Counter(
            (str(o.device), str(o.dtype), tuple(o.shape))  # 将 o.dtype 转换为字符串
            for o in gc.get_objects()
            if torch.is_tensor(o)
        )
        
        # 注意，这里 x[0] 是 (device, dtype, shape) 的三元组，x[1] 是计数
        for line in sorted(tensors.items(), key=lambda x: (x[0][0], x[0][1], x[0][2])):
            print('{}\t{}'.format(*line))

    def pgd_sds_hallo(self, X, net, c, label=None, random_start=False, delta_score=False, name='test', diff_pgd=False, using_target=False, target_info=None, target_rate=None):
        # add uniform random start [-eps, eps] default: eps=0.12549019607843137[32/255]
        # print(X['image'].min(),X['image'].max()) # [-1, 1]

        X['image'] = X['image'].detach()
        X_adv = copy.deepcopy(X)
        X_adv['image'] = X_adv['image'] + (torch.rand(*X['image'].detach().shape)*2*self.eps-self.eps).cuda()
        ...
        for i in pbar:
            ...
            grad, = torch.autograd.grad(loss, [X_adv['image']], retain_graph=False)
            
            # update
            X_adv['image'] = X_adv['image'] + self.g_dir * grad.detach().sign() * actual_step_size
            # clip
            X_adv['image'] = torch.minimum(torch.maximum(X_adv['image'], X['image'].detach() - self.eps), X['image'].detach() + self.eps)
            X_adv['image'] = torch.clamp(X_adv['image'], min=self.clip_min, max=self.clip_max).detach()
            X_adv['image'].grad = None
            self.net.model.net.reference_control_reader.clear()
            self.net.model.net.reference_control_writer.clear()
        return X_adv['image']
