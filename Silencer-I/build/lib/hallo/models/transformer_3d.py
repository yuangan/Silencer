...
class Transformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        hidden_states = rearrange(hidden_states, "b c f h w -> (b f) c h w") # 2, 320, 16, 32, 32
        
        # TODO
        if self.use_audio_module:
            # print('encoder_hidden_states 2: ', encoder_hidden_states.shape) # 2, 16, 32, 768
            # margin=context_dim?
            encoder_hidden_states = rearrange(
                encoder_hidden_states,
                "bs f margin dim -> (bs f) margin dim",
            )
            # print('encoder_hidden_states 3: ', encoder_hidden_states.shape) # 32, 32, 768
        else:
        ...
        # print('encoder_hidden_states 4: ', encoder_hidden_states.shape)  # 32, 32, 768
        ...
        if not self.use_linear_projection:
            ...
            hidden_states = self.proj_in(hidden_states)
        # print(inner_dim, hidden_states.shape) # 320 torch.Size([32, 4096, 320])
        # print(self.use_linear_projection) # False
        # assert(0)
        # Blocks
        motion_frames = []
