...
class ImageProcessor:
    ...
    def preprocess_target_img(self, source_image_path: str, cache_dir: str, face_region_ratio: float):
        """
        ...
        return pixel_values_ref_img, face_emb

    def preprocess_face_emb(self, source_image):
        img = (source_image[0].permute(1,2,0).detach().cpu().numpy() + 1) * 127.5  #  [0, 255] 
        img = np.clip(img, 0, 255).astype(np.uint8)  #  uint8 
        ref_image_pil = Image.fromarray(img)
        faces = self.face_analysis.get(cv2.cvtColor(np.array(ref_image_pil.copy()), cv2.COLOR_RGB2BGR))
    ...
    def preprocess(self, source_image_path: str, cache_dir: str, face_region_ratio: float):
        ...
        source_image = Image.open(source_image_path)
        ref_image_pil = source_image.convert("RGB")
        # print(ref_image_pil.size, np.array(ref_image_pil).max(), np.array(ref_image_pil).min()) # (512, 512) 255 0
        # 1. image augmentation
        pixel_values_ref_img = self._augmentation(ref_image_pil, self.pixel_transform)
        # print(pixel_values_ref_img.shape, pixel_values_ref_img.max(), pixel_values_ref_img.min()) # [3, 512, 512] tensor(1.) tensor(-1.)
