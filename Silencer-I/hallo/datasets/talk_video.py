...
class TalkingVideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(audio_emb.shape) # [n 12 768]

        ref_img_idx = random.randint(
            self.n_motion_frames,
            video_length - self.n_sample_frames - self.audio_margin - 1,
        ) # random pick a frame as ref_image from [n_motion_frames, video_length - self.n_sample_frames - self.audio_margin - 1]

        ref_img = video_frames[ref_img_idx].asnumpy()
        ref_img = Image.fromarray(ref_img)

        if self.n_motion_frames > 0: # motion frames is n frames before start_idx [have no bussiness with ref_img_idx]
        ...
        # print((tgt_mask_pil).size, pixel_values_mask.shape) # (384, 382) torch.Size([3, 512, 512])
        ...
        if self.n_motion_frames > 0: # add [start_idx - self.n_motion_frames : start_idx] frames as motion
        ...
        # print(pixel_values_ref_img.shape) # [1+n_motion_frames, 3, 512, 512]
